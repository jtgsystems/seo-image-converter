...
class SEOImageConverterGUI:
    ...
    def __init__(self):
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=2, thread_name_prefix="GUI-Worker"
        )
    ...
    def _setup_themes(self):
        ...
        with dpg.font_registry():
            ...
            try:
                ...
                logger.warning(
                    f"Could not load custom font, using system default: {font_error}"
                )
    ...
    def _create_header(self):
        ...
        dpg.add_text(
            "AI-Powered Image Optimization • Dear PyGui 2.1.0 • 2025",
            color=(150, 150, 150),
        )
        dpg.add_separator()
    ...
    def _create_input_section(self):
        ...
        with dpg.collapsing_header(
            label="📁 Input & Output Directories", default_open=True
        ):
            # Input directory
            with dpg.group(horizontal=True):
                dpg.add_text("Input Directory:", width=120)
                self.input_dir = dpg.add_input_text(
                    hint="Select directory containing images...", width=-150
                )
                dpg.add_button(
            ...
            with dpg.group(horizontal=True):
                ...
                self.output_dir = dpg.add_input_text(
                    hint="Leave empty to use input directory", width=-150
                )
                dpg.add_button(
        ...
        with dpg.collapsing_header(label="🎛️ Processing Options", default_open=True):
            ...
            with dpg.group(horizontal=True):
                ...
                self.format_combo = dpg.add_combo(
                    ["webp", "png", "jpeg"], default_value="webp", width=120
                )
            ...
            with dpg.group(horizontal=True):
                ...
                self.dry_run_checkbox = dpg.add_checkbox(
                    label="🧪 Preview mode", default_value=False
                )
    ...
    def _create_results_section(self):
        ...
        with dpg.table(
            ...
            dpg.add_table_column(
                label="Original File", width_stretch=True, init_width_or_weight=200
            )
            dpg.add_table_column(
                label="SEO Keywords", width_stretch=True, init_width_or_weight=300
            )
            dpg.add_table_column(
                label="Size Reduction", width_fixed=True, init_width_or_weight=120
            )
    ...
    def _check_ollama_status(self):
        ...
        def check_status():
            try:
                ...
                def update_error():
                    dpg.set_value(
                        self.ollama_status, "❌ AI Service: Connection Failed"
                    )
    ...
    def _browse_input_directory(self):
        ...
        try:
            ...
            directory = filedialog.askdirectory(
                title="Select Input Directory", mustexist=True
            )
            root.destroy()
    ...
    def _browse_output_directory(self):
        ...
        try:
            ...
            directory = filedialog.askdirectory(
                title="Select Output Directory", mustexist=True
            )
            root.destroy()
        ...
        try:
            ...
            if not input_dir:
                self._show_modal_error(
                    "Input Error", "Please select an input directory"
                )
                return
            ...
            future = self.executor.submit(
                self._process_images_thread, input_path, output_path
            )
            self.pending_futures.add(future)
    ...
    def _processing_complete(self, report: dict):
        ...
        dpg.set_value(
            self.savings_text, f"Saved: {savings_mb:.1f} MB ({compression:.1f}%)"
        )
        ...
        if summary["failed"] == 0:
            self._show_modal_info(
                "Success!", f"✅ Successfully processed {summary['processed']} images"
            )
        else:
            self._show_modal_warning(
                ...
                if output_dir and Path(output_dir).exists():
                    ...
                    try:
                        if system == "Linux":
                            ...
                            self._show_modal_warning(
                                "Unsupported OS", f"Cannot open folder on {system}"
                            )
                    except Exception as open_error:
                        logger.error(f"Failed to open folder: {open_error}")
                else:
                    self._show_modal_warning(
                        "Folder Not Found", "Output directory does not exist"
                    )
    ...
    def _show_modal_warning(self, title: str, message: str):
        ...
        with dpg.window(
            ...
            height=200,
            pos=[400, 300],
            tag=f"warning_modal_{time.time()}",
        ):
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Forced executor shutdown after timeout: {shutdown_error}"
                )
