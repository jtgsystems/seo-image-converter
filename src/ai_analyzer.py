...
Your response must ONLY contain the {self.keyword_count} keywords separated by hyphens, nothing else."""
    ...
    def _call_ollama_api(self, payload: Dict, image_path: Path) -> Optional[str]:
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                logger.debug(f"Calling Ollama API (attempt {attempt}/{self.max_retries}) for {image_path.name}")
                ...
                if response.status_code == 200:
                    ...
                    if response_text:
                        ...
                        logger.warning(f"Empty response from Ollama for {image_path.name}")
                
                else:
                    logger.warning(f"Ollama API returned status {response.status_code}: {response.text}")
                
            except requests.exceptions.Timeout:
                logger.warning(f"Timeout calling Ollama API for {image_path.name} (attempt {attempt})")
                
            except requests.exceptions.RequestException as e:
                logger.warning(f"Request failed for {image_path.name} (attempt {attempt}): {e}")
                
            except json.JSONDecodeError as e:
                logger.warning(f"Invalid JSON response for {image_path.name} (attempt {attempt}): {e}")
                
            except Exception as e:
                logger.error(f"Unexpected error calling Ollama API for {image_path.name} (attempt {attempt}): {e}")
            
            # Wait before retry (except on last attempt)
            if attempt < self.max_retries:
                time.sleep(self.retry_delay * attempt)  # Exponential backoff
        
        logger.error(f"Failed to get response from Ollama after {self.max_retries} attempts for {image_path.name}")
        return None
    ...
    def _process_seo_response(self, response_text: str) -> str:
        ...
        max_length = self.seo_config.get('max_filename_length', 100) - 5  # Reserve space for .webp
    ...
    def generate_fallback_name(self, image_path: Path) -> str:
        ...
        # Clean original stem
        clean_stem = ''.join(c if c.isalnum() or c == '-' else '-' for c in original_stem)
    ...
    def batch_analyze_images(self, image_paths: List[Path], progress_callback=None) -> Dict[Path, Optional[str]]:
