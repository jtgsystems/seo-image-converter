...
class SEOImageConverterGUI:
    ...
    def _create_gui(self):
        ...
        with dpg.window(label="SEO Image Converter", tag="main_window"):
            # Header
            dpg.add_text(
                "🚀 SEO Image Converter - Latest 2025 Framework", color=[66, 150, 250]
            )
            dpg.add_text(
                "AI-Powered Image Optimization with Qwen2.5-VL + Dear PyGui 2.1.0",
                color=[100, 100, 100],
            )
            dpg.add_separator()
            ...
            with dpg.group(horizontal=True):
                self.input_dir = dpg.add_input_text(
                    hint="Select directory containing images...", width=600
                )
                dpg.add_button(label="Browse", callback=self._browse_input_directory)
            ...
            with dpg.group(horizontal=True):
                self.output_dir = dpg.add_input_text(
                    hint="Leave empty to use input directory", width=600
                )
                dpg.add_button(label="Browse", callback=self._browse_output_directory)
            ...
            with dpg.group(horizontal=True):
                ...
                self.format_combo = dpg.add_combo(
                    ["webp", "png", "jpeg"], default_value="webp", width=100
                )

                dpg.add_text("Quality:")
                self.quality_slider = dpg.add_slider_int(
                    min_value=50, max_value=100, default_value=85, width=150
                )
            ...
            with dpg.group(horizontal=True):
                ...
                self.skip_existing_checkbox = dpg.add_checkbox(
                    label="⏭️ Skip existing", default_value=False
                )
                self.dry_run_checkbox = dpg.add_checkbox(
                    label="🧪 Dry run", default_value=False
                )
            ...
            self.progress_bar = dpg.add_progress_bar(
                overlay="Ready to process images...", width=-1
            )
        ...
        def check():
            try:
                ...
                if available:
                    ...
                    dpg.set_value(
                        self.ollama_status, "⚠️ Ollama AI Service: Disconnected"
                    )
    ...
    def _start_processing(self):
        ...
        try:
            ...
            config.update(
                f"image.quality.{dpg.get_value(self.format_combo)}",
                dpg.get_value(self.quality_slider),
            )
            config.update(
    ...
    def _processing_complete(self, report: dict):
        ...
        dpg.set_value(
            self.savings_text,
            f"Savings: {savings_mb:.1f} MB ({size_analysis['overall_compression_ratio']:.1f}%)",
        )
    ...
    def _show_error(self, message: str):
        """Show error popup."""
        with dpg.window(
            label="Error", modal=True, width=400, height=150, pos=[300, 250]
        ):
    ...
    def _show_info(self, message: str):
        """Show info popup."""
        with dpg.window(
            label="Information", modal=True, width=400, height=150, pos=[300, 250]
        ):
