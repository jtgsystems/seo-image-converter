...
class SEOImageProcessor:
    ...
    def process_single_image(
        self, input_path: Path, output_dir: Path, backup_dir: Optional[Path] = None
    ) -> ProcessingResult:
        ...
        try:
            ...
            if not seo_keywords:
                logger.warning(
                    f"Failed to generate SEO keywords for {input_path}, using fallback"
                )
            ...
            success, optimization_result = self.optimizer.optimize_image(
                input_path, output_path
            )

            if not success:
                error_msg = optimization_result.get(
                    "error", "Unknown optimization error"
                )
            ...
            if (
                backup_dir and input_path != output_path
            ):  # Don't backup if processing in-place
                try:
            ...
            logger.info(
                f"Successfully processed: {input_path.name} -> {output_path.name} "
    ...
    def _process_images_parallel(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            future_to_path = {
                executor.submit(
                    self.process_single_image, img_path, output_dir, backup_dir
                ): img_path
            ...
            for future in as_completed(future_to_path):
                ...
                try:
                    ...
                    if completed_count % 10 == 0 or completed_count == total_count:
                        logger.info(
                            f"Progress: {completed_count}/{total_count} images processed"
                        )
    ...
    def _create_final_report(self, results: List[ProcessingResult]) -> Dict:
        ...
        report = {
            "summary": {
                ...
                "average_time_per_image": processing_time
            ...
            "size_analysis": {
                ...
                "average_compression_per_image": sum(
                    r.compression_ratio for r in results if r.success
                )
