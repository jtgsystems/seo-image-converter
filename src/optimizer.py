...
class ImageOptimizer:
    ...
    def optimize_image(self, input_path: Path, output_path: Path) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            if self.output_format.lower() == "webp":
                ...
                return False, {
                    "error": f"Unsupported output format: {self.output_format}"
                }
    ...
    def _optimize_to_webp(
        self, input_path: Path, output_path: Path, image_info: Dict
    ) -> bool:
        ...
        try:
            ...
            try:
                ...
                if len(optimized_data) < len(original_data):
                    ...
                    logger.debug(
                        f"Zopfli optimization saved {len(original_data) - len(optimized_data)} bytes"
                    )
