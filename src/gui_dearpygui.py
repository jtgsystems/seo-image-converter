...
class SEOImageConverterGUI:
    ...
    def _create_header(self):
        ...
        with dpg.group(horizontal=True):
            ...
            self.ollama_status = dpg.add_text(
                "⚠️ Checking AI service...", color=(255, 193, 7)
            )
        ...
        with dpg.collapsing_header(label="📁 Directory Selection", default_open=True):
            with dpg.group(horizontal=True):
                ...
                self.input_dir_text = dpg.add_input_text(
                    hint="Select or drag & drop image directory...", width=-150
                )
                dpg.add_button(
                    label="Browse", callback=self._browse_input_directory, width=130
                )
            ...
            with dpg.group(horizontal=True):
                ...
                self.output_dir_text = dpg.add_input_text(
                    hint="(Same as input if empty)", width=-150
                )
                dpg.add_button(
                    label="Browse", callback=self._browse_output_directory, width=130
                )
            ...
            with dpg.table(
                ...
                with dpg.table_row():
                    ...
                    self.format_combo = dpg.add_combo(
                        items=["webp", "png", "jpeg"], default_value="webp", width=100
                    )
                    dpg.add_spacer()
    ...
    def _create_results_tab(self):
        ...
        with dpg.collapsing_header(label="📊 Summary", default_open=True):
            with dpg.table(header_row=True, borders_innerV=True):
                dpg.add_table_column(
                    label="Metric", width_fixed=True, init_width_or_weight=150
                )
        ...
        with dpg.collapsing_header(label="📋 Detailed Results", default_open=True):
            with dpg.table(
                ...
                dpg.add_table_column(
                    label="Status", width_fixed=True, init_width_or_weight=60
                )
                dpg.add_table_column(
                    label="Input File", width_fixed=True, init_width_or_weight=200
                )
                dpg.add_table_column(
                    label="SEO Keywords", width_fixed=True, init_width_or_weight=250
                )
                dpg.add_table_column(
                    label="Original Size", width_fixed=True, init_width_or_weight=100
                )
                dpg.add_table_column(
                    label="New Size", width_fixed=True, init_width_or_weight=100
                )
                dpg.add_table_column(
                    label="Savings", width_fixed=True, init_width_or_weight=80
                )
    ...
    def _create_settings_tab(self):
        ...
        with dpg.collapsing_header(label="🤖 AI Configuration", default_open=True):
            ...
            dpg.add_input_text(
                default_value=config.ollama.get(
        ...
        with dpg.collapsing_header(label="🖼️ Image Processing", default_open=True):
            ...
            dpg.add_input_int(
                default_value=config.image.get("optimization", {}).get(
                    "max_dimension", 1920
                ),
                width=-1,
                tag="max_dimension",
        ...
        with dpg.group(horizontal=True):
            dpg.add_button(
                label="💾 Save Settings", callback=self._save_settings, width=150
            )

            dpg.add_same_line(spacing=20)

            dpg.add_button(
                label="🔄 Reset to Defaults", callback=self._reset_settings, width=150
            )
    ...
    def _create_logs_tab(self):
        ...
        with dpg.group(horizontal=True):
            dpg.add_button(label="🗑️ Clear Logs", callback=self._clear_logs, width=100)

            dpg.add_same_line(spacing=10)

            dpg.add_button(label="💾 Save Logs", callback=self._save_logs, width=100)
        ...
        with dpg.child_window(height=-1, border=True, tag="logs_window"):
            self.log_text = dpg.add_text(
                "Application logs will appear here...", wrap=-1
            )
    ...
    def _process_images_thread(self, input_dir: Path, output_dir: Optional[Path]):
        ...
        try:
            ...
            def progress_callback(completed: int, total: int, result: ProcessingResult):
                ...
                overlay_text = (
                    f"Processing: {completed}/{total} ({progress * 100:.0f}%)"
                )
    ...
    def _update_results_display(self, report: dict):
        ...
        try:
            ...
            dpg.set_value(
                ...
                f"{size_analysis['overall_compression_ratio']:.1f}%",
            )
    ...
    def _apply_settings(self):
        ...
        try:
            ...
            # Processing options
            config.update(
                "processing.backup_originals", dpg.get_value(self.backup_checkbox)
            )
            config.update(
                "processing.skip_existing", dpg.get_value(self.skip_existing_checkbox)
            )
    ...
    def _save_settings(self):
        ...
        try:
            ...
            if dpg.does_item_exist("max_dimension"):
                config.update(
                    "image.optimization.max_dimension", dpg.get_value("max_dimension")
                )
            if dpg.does_item_exist("strip_metadata"):
                config.update(
                    "image.optimization.strip_metadata", dpg.get_value("strip_metadata")
                )
            if dpg.does_item_exist("progressive_jpeg"):
                config.update(
                    "image.optimization.progressive", dpg.get_value("progressive_jpeg")
                )
        ...
        try:
            ...
            if output_dir and Path(output_dir).exists():
                ...
                try:
                    ...
                    logger.error(f"Failed to open folder: {open_error}")
                    ...
                    for result in self.current_results["detailed_results"]:
                        writer.writerow(
                            [
                                ...
                                f"{result['optimized_size'] / 1024 / 1024:.2f}"
                                if result["optimized_size"] > 0
                                else "N/A",
                                f"{result['compression_ratio']:.1f}"
                                if result["success"]
    ...
    def _show_error(self, title: str, message: str):
        ...
        with dpg.window(
            ...
            dpg.add_separator()
            dpg.add_button(
                label="OK", callback=lambda: dpg.delete_item("error_modal"), width=-1
            )
    ...
    def _show_warning(self, title: str, message: str):
        ...
        with dpg.window(
            ...
            dpg.add_separator()
            dpg.add_button(
                label="OK", callback=lambda: dpg.delete_item("warning_modal"), width=-1
            )
    ...
    def _show_info(self, title: str, message: str):
        ...
        with dpg.window(
            ...
            dpg.add_separator()
            dpg.add_button(
                label="OK", callback=lambda: dpg.delete_item("info_modal"), width=-1
            )
