...
class ProgressTracker:
    ...
    def __enter__(self):
        ...
        self.task = self.progress.add_task(
            "Processing images...", total=self.total_files
        )
        self.progress.start()
        return self
    ...
    def update(self, completed: int, total: int, result: ProcessingResult):
        ...
        if self.progress and self.task:
            ...
            description = (
                f"Processing images... {status_emoji} {result.input_path.name}"
            )

            self.progress.update(
                self.task, completed=completed, description=description
            )
...
def display_summary_report(report: dict):
    ...
    summary_table.add_row(
        "⚡ Avg Time/Image", f"{summary['average_time_per_image']:.2f}s"
    )
    ...
    size_table.add_row(
        "Original Total Size", format_file_size(size_analysis["total_original_size"])
    )
    size_table.add_row(
        "Optimized Total Size", format_file_size(size_analysis["total_optimized_size"])
    )
    size_table.add_row(
        "💾 Total Savings",
        f"[green]{format_file_size(size_analysis['total_savings_bytes'])}[/green]",
    )
    size_table.add_row(
        "📊 Overall Compression",
        f"[green]{size_analysis['overall_compression_ratio']:.1f}%[/green]",
    )
    size_table.add_row(
        "📈 Avg Compression/Image",
        f"{size_analysis['average_compression_per_image']:.1f}%",
    )
...
def display_detailed_results(results: list, show_all: bool = False):
    ...
    for result in display_results:
        ...
        optimized_size = (
            format_file_size(result["optimized_size"])
...
@click.option(
    "--detailed", is_flag=True, help="Show detailed results for all processed images"
)
@click.option(
    "--json-output",
    type=click.Path(path_type=Path),
