...
class SEOImageConverterGUI:
    ...
    def __init__(self):
        ...
        # Application state
        self.processor = SEOImageConverter()
    ...
    def _create_header(self):
        ...
        if HAS_MODERN_GUI:
            ...
            title_label = ttk.Label(header_frame, text="SEO Image Converter", font=("Arial", 16, "bold"))
        ...
        if HAS_MODERN_GUI:
            ...
            config_btn = ttk.Button(header_frame, text="Settings", command=self._open_settings)
    ...
    def _create_directory_section(self, parent):
        ...
        if HAS_MODERN_GUI:
            ctk.CTkLabel(input_frame, text="📁 Input Directory:").grid(row=0, column=0, padx=10, pady=10, sticky="w")
            ...
            ttk.Label(input_frame, text="Input Directory:").grid(row=0, column=0, padx=10, pady=5, sticky="w")
        ...
        if HAS_MODERN_GUI:
            ...
            browse_btn = ttk.Button(input_frame, text="Browse", command=self._browse_input_directory)
        ...
        if HAS_MODERN_GUI:
            ctk.CTkLabel(output_frame, text="💾 Output Directory:").grid(row=0, column=0, padx=10, pady=10, sticky="w")
        else:
            ttk.Label(output_frame, text="Output Directory:").grid(row=0, column=0, padx=10, pady=5, sticky="w")
        ...
        if HAS_MODERN_GUI:
            ...
            output_browse_btn = ttk.Button(output_frame, text="Browse", command=self._browse_output_directory)
    ...
    def _create_options_section(self, parent):
        ...
        # Format selection
        if HAS_MODERN_GUI:
            ctk.CTkLabel(options_frame, text="🖼️ Output Format:").grid(row=0, column=0, padx=10, pady=5, sticky="w")
            ...
            ttk.Label(options_frame, text="Output Format:").grid(row=0, column=0, padx=10, pady=5, sticky="w")
            
            self.format_var = tk.StringVar(value="webp")
            format_menu = ttk.Combobox(options_frame, textvariable=self.format_var, values=["webp", "png", "jpeg"], state="readonly")
            
        format_menu.grid(row=0, column=1, padx=10, pady=5, sticky="w")
        
        # Quality setting
        if HAS_MODERN_GUI:
            ctk.CTkLabel(options_frame, text="🎛️ Quality:").grid(row=0, column=2, padx=10, pady=5, sticky="w")
            ...
            ttk.Label(options_frame, text="Quality:").grid(row=0, column=2, padx=10, pady=5, sticky="w")
            
            self.quality_var = tk.IntVar(value=85)
            quality_slider = ttk.Scale(options_frame, from_=50, to=100, variable=self.quality_var, orient="horizontal")
        ...
        checkbox_frame.grid(row=1, column=0, columnspan=5, sticky="ew", padx=10, pady=10)
        ...
        if HAS_MODERN_GUI:
            backup_check = ctk.CTkCheckBox(checkbox_frame, text="💾 Backup originals", variable=self.backup_var)
            skip_check = ctk.CTkCheckBox(checkbox_frame, text="⏭️ Skip existing files", variable=self.skip_existing_var)
            dry_run_check = ctk.CTkCheckBox(checkbox_frame, text="🧪 Dry run (preview only)", variable=self.dry_run_var)
        else:
            backup_check = ttk.Checkbutton(checkbox_frame, text="Backup originals", variable=self.backup_var)
            skip_check = ttk.Checkbutton(checkbox_frame, text="Skip existing files", variable=self.skip_existing_var)
            dry_run_check = ttk.Checkbutton(checkbox_frame, text="Dry run (preview only)", variable=self.dry_run_var)
    ...
    def _create_progress_section(self, parent):
        ...
        if HAS_MODERN_GUI:
            self.progress_label = ctk.CTkLabel(label_frame, text="Ready to process images...")
            self.stats_label = ctk.CTkLabel(label_frame, text="")
        else:
            self.progress_label = ttk.Label(label_frame, text="Ready to process images...")
    ...
    def _create_control_buttons(self, parent):
        ...
        if HAS_MODERN_GUI:
            ...
            self.stop_button = ttk.Button(button_frame, text="Stop", command=self._stop_processing, state="disabled")
    ...
    def _create_results_table(self, parent):
        ...
        columns = ("Status", "Input File", "SEO Keywords", "Original Size", "New Size", "Savings")
    ...
    def _start_processing(self):
        ...
        output_dir = Path(self.output_dir_var.get()) if self.output_dir_var.get() != "(Same as input)" else None
    ...
    def _process_images_thread(self, input_dir: Path, output_dir: Optional[Path]):
        ...
        try:
            def progress_callback(completed: int, total: int, result: ProcessingResult):
                ...
                self.root.after(0, lambda: self._update_progress(progress, completed, total, result))
    ...
    def _update_progress(self, progress: float, completed: int, total: int, result: ProcessingResult):
